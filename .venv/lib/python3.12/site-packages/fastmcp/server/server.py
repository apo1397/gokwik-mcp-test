        ...
        async with fastmcp.server.context.Context(fastmcp=self) as ctx:
            ...
            with server_span(
                ...
                try:
                    ...
                    logger.exception(f"FastMCPError in tool {name!r} with arguments {arguments!r}")
                    raise
                except (ValidationError, PydanticValidationError) as e:
                    logger.exception(f"ValidationError in tool {name!r} with arguments {arguments!r}: {e}")
                    raise
                except Exception as e:
                    logger.exception(f"Unexpected error calling tool {name!r} with arguments {arguments!r}: {e}")
                    # Handle actionable errors that should reach the LLM
                    # even when masking is enabled
                    if isinstance(e, httpx.HTTPStatusError):
                        if e.response.status_code == 429:
                            raise ToolError(
                                f"Rate limited by upstream API in tool {name!r}, please retry later"
                            ) from e
                    if isinstance(e, httpx.TimeoutException):
                        raise ToolError(
                            f"Upstream request timed out in tool {name!r}, please retry"
                        ) from e
                    # Standard masking logic
                    if self._mask_error_details:
                        raise ToolError(f"Error calling tool {name!r}. Check logs for details.") from e
                    raise ToolError(f"Error calling tool {name!r} with arguments {arguments!r}: {e}") from e
